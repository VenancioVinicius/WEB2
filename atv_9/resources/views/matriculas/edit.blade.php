...
@extends('templates.main', ['titulo' => "Alterar Docencia"])
    ...
    <div class="row">
        <div class="col" >
            <div class="form-floating mb-3">
                ...
                <select name="id_diciplina" class="form-control {{ $errors->has('id_diciplina') ? 'is-invalid' : '' }}">
                    @foreach ($diciplina as $item) 
                        <option value="{{$item->id}}" @if($item->id == $dados['id_diciplina']) selected="true" @endif>
                            {{$item->nome}}
                        </option>
                    @endforeach
                </select>
                @if($errors->has('id_diciplina'))
                    <div class='invalid-feedback'>
                        {{ $errors->first('id_diciplina') }}
                    </div>
                @endif
            </div>
        </div>
